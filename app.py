...
rows = []
rows.append({"Estaca":f"{pcv_s}+{pcv_o:.2f}","Posição":round(PCV_chain,3),"Dist_A":0.0,"Cota":round(Z_A,3),"Tipo":"PCV"})
...
for s in range(start_i,end_i+1):
    ...
    rows.append({"Estaca":f"{s}+00","Posição":ch,"Dist_A":round(dx,3),"Cota":round(Z_A + (i1*dx - (g/(2*L))*dx**2),3),"Tipo":""})
...
for s in range(station_i+1, math.floor(PTV_chain/20)+1):
    ch=s*20; dx=ch-PCV_chain
    rows.append({"Estaca":f"{s}+00","Posição":ch,"Dist_A":round(dx,3),"Cota":round(Z_A + (i1*dx - (g/(2*L))*dx**2),3),"Tipo":""})
rows.append({"Estaca":f"{ptv_s}+{ptv_o:.2f}","Posição":round(PTV_chain,3),"Dist_A":round(L,3),"Cota":round(Z_B,3),"Tipo":"PTV"})
...
def create_pdf():
    ...
    pdf.cell(0, 8, f"Estaca PIV: {station_i}+{offset_i:.2f}", ln=True)
    ...
    for row in rows:
        ...
        pdf.cell(col_width, 8, str(row["Posição"]), border=1)
        pdf.cell(col_width, 8, str(row["Dist_A"]), border=1)
        pdf.cell(col_width, 8, str(row["Cota"]), border=1)
        pdf.cell(col_width, 8, row["Tipo"], border=1)
        pdf.ln()
    pdf.ln(5)
    buf = BytesIO()
